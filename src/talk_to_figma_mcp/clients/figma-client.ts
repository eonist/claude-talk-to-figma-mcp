  ...
  async executeCommand(command: FigmaCommand, params: any = {}): Promise<any> {
    try {
      ...
      if (command === 'get_document_info' || command === 'get_selection') {
        ...
        // Log the raw structure for diagnostic purposes
        logger.info(`[DIAGNOSTIC] Examining result structure for ${command}:`);
        logger.info(`[DIAGNOSTIC] - Type: ${typeof typedResult}`);
        logger.info(`[DIAGNOSTIC] - Is Object: ${typedResult && typeof typedResult === 'object'}`);
        if (typedResult && typeof typedResult === 'object') {
          logger.info(`[DIAGNOSTIC] - Keys: ${Object.keys(typedResult).join(', ')}`);
          
          if (command === 'get_document_info') {
            logger.info(`[DIAGNOSTIC] - Has name: ${Boolean(typedResult.name)}`);
            logger.info(`[DIAGNOSTIC] - Has children: ${Boolean(typedResult.children)}`);
            logger.info(`[DIAGNOSTIC] - Has currentPage: ${Boolean(typedResult.currentPage)}`);
            if (typedResult.children) {
              logger.info(`[DIAGNOSTIC] - Children length: ${typedResult.children.length}`);
            }
          }
          
          if (command === 'get_selection') {
            logger.info(`[DIAGNOSTIC] - Has selectionCount: ${Boolean(typedResult.selectionCount)}`);
            logger.info(`[DIAGNOSTIC] - Has selection: ${Boolean(typedResult.selection)}`);
            if (typedResult.selection) {
              logger.info(`[DIAGNOSTIC] - Selection length: ${typedResult.selection.length}`);
            }
          }
        }
        
        // IMPORTANT: Check if we have ACTUAL data - the terminal logs show that the data IS coming in
        // but our previous check might be wrong. Let's be more specific about what constitutes "real data"
        const hasRealDocumentData = typedResult?.currentPage || 
                                     (typedResult?.children && Array.isArray(typedResult.children) && typedResult.children.length > 0);
                                     
        const hasRealSelectionData = (typedResult?.selection && Array.isArray(typedResult.selection) && typedResult.selection.length > 0);
        
        // Add payload size information to our diagnostic logs
        const payloadSize = JSON.stringify(typedResult).length;
        logger.info(`[DIAGNOSTIC] Payload size: ${payloadSize} characters`);
        ...
        if (typedResult && typeof typedResult === 'object' && typedResult.success === true && 
            !hasRealDocumentData && !hasRealSelectionData) {
          
          logger.warn(`Received simplified success object instead of full data for ${command}. Returning enhanced version.`);
          logger.warn(`[DIAGNOSTIC] Reason: No real data found in the payload. Size: ${payloadSize} chars`);
          
          // For document info, return a minimal document structure with the command info AND diagnostics
          if (command === 'get_document_info') {
            return {
              name: "Document",
              id: "document-info",
              ...
              _debug: {
                timestamp: Date.now(),
                message: `Minimal document info returned due to data loss in transmission. Payload size: ${payloadSize} chars`,
                actualKeys: typedResult ? Object.keys(typedResult) : []
              }
            };
          }
          
          // For selection, return a minimal selection structure with the command info AND diagnostics
          if (command === 'get_selection') {
            return {
              selectionCount: 0,
              selection: [],
              ...
              _debug: {
                timestamp: Date.now(),
                message: `Minimal selection info returned due to data loss in transmission. Payload size: ${payloadSize} chars`,
                actualKeys: typedResult ? Object.keys(typedResult) : []
              }
            };
          }
