 ...
 * @async
 * @function
 * @param {Object|Object[]} params - Button configuration parameters or { buttons: [...] }
 * @returns {Promise<Array|Object>} Array of results for batch, or single result for single.
 */
export async function createButton(params) {
  // Batch support: if params.buttons exists, treat as batch
  if (Array.isArray(params.buttons)) {
    const results = [];
    for (const btn of params.buttons) {
      results.push(await createButton(btn));
    }
    return results;
  }
